...
def process_pmid(pmid, pubtator_path, output_path, port, llm_model_name, embeddings_model_name):
    """
    ...
    try:
        ...
        llm = ChatOllama(
            ...
            temperature=0,
            base_url=f"http://127.0.0.1:{port}" # Default Ollama base URL
        )
        # llm = True # Keep this if you want to test without actual LLM calls
...
def main():
    ...
    # --- Argument Parsing ---
    parser = argparse.ArgumentParser(description="Process PMIDs with iText2KG.")
    parser.add_argument('--port', type=int, default=11434, help='Port for Ollama server (default: 11434)')
    parser.add_argument('--num', type=int, default=1, help='Port for Ollama server (default: 11434)')
    ...
    pmid_list = pmid_list_chunk[args.num] # Use the chunk based on the argument
    tasks = pmid_list
